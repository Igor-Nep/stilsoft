...
class ApiMurom:
    ...
    def small_token(self): #только тела токена
        creds = {       
            'login': 'admin',
            'password': 'adm777'
            }
    ...
    def needed_notifications(self):
            ...
            e_codes = [1350,3350,1322,3322,13221,3321,1352,1792,3792,1323,3323,1784,1352,1779,1780,3780,1781,1782,1783,1135,3135,1321,3321,1322,3322,1323,3323,1324,3324,1502,1503,1504,1505,1397,3350,1392,3392,1394,3394,1395,3395,1396,1399,3399,1911,3911,1912,3912,1720,3720,4730,5720,4731,1850,3850,3851,1851,1857,1920,1921,1922,1923,1924,3923,1930,1931,1932,1933,1351,3351,3352,1130,3130,1137,3137,1302,3302,1338,3338,1912,1911,1913,1914,3914,1910,3910]
            #e_codes = [1350]    
            d = int(input(f'Введите номер устройства из списка: '))
            device = (self.get_module(d)['id'])
    ...
    def notifications(self): #вывод нотификаций
            ...
            self.refresh_token()
            for _ in range (0, i):
                resp = requests.post(self.url+'/api/data/events/history', headers=self.get_token(), json=body, verify=False) 
                print(resp)
    ...
    def get_8122(self): #получение id камеры 8122
    ...
    def get_camera(self): #получение id камеры

        warnings.filterwarnings('ignore')
        for i in range (self.get_module_count()):
            sts_ch = (self.get_modules()['data'][i]['title'])
            if sts_ch == 'Камера дальнего обзора':
                d = i
    ...
    def check_8122_state(self):
        ...
        nul={}
        msp_null = msgpack.packb(nul)
        resp = requests.post(self.url+f'/api/modules/{self.get_8122()}/ModuleStateRequest',headers=head, data=msp_null, verify=False )
    ...
    def add_user_by_input(self): #добавление пользователя с input
        warnings.filterwarnings('ignore')
    ...
    def del_user_by_id(self, id):
        warnings.filterwarnings('ignore')
        requests.delete(self.url+f'/api/data/security/user/{id}', headers=self.get_token(), verify=False)
